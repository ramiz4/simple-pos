...
export class LoggerService {
  ...
  private async log(
    ...
    try {
      ...
      const entry: ErrorLogEntry = {
        ...
        stack: this.extractStack(context),
  ...
  private extractStack(context?: Record<string, unknown>): string | undefined {
    if (!context) return undefined;

    const directStack = context['stack'];
    if (typeof directStack === 'string') return directStack;

    const errorValue = context['error'];
    if (errorValue instanceof Error && typeof errorValue.stack === 'string') {
      return errorValue.stack;
    }

    if (typeof errorValue === 'object' && errorValue !== null) {
      const stackValue = (errorValue as { stack?: unknown }).stack;
      if (typeof stackValue === 'string') return stackValue;
    }
