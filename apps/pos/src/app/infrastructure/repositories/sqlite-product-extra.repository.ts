...
export class SQLiteProductExtraRepository implements ProductExtraRepository {
  private db: Database | null = null;

  async findById(id: number): Promise<ProductExtra | null> {
    const db = await this.getDb();
    const results = await db.select<ProductExtra[]>('SELECT * FROM product_extra WHERE id = ?', [
      id,
    ]);
  ...
  async findByProduct(productId: number): Promise<ProductExtra[]> {
    const db = await this.getDb();
    return await db.select<ProductExtra[]>('SELECT * FROM product_extra WHERE productId = ?', [
      productId,
    ]);
  }
  ...
  async update(id: number, entity: Partial<ProductExtra>): Promise<ProductExtra> {
    ...
    await db.execute('UPDATE product_extra SET productId = ?, extraId = ? WHERE id = ?', [
      updated.productId,
      updated.extraId,
      id,
    ]);
  ...
  async deleteByProductAndExtra(productId: number, extraId: number): Promise<void> {
    const db = await this.getDb();
    await db.execute('DELETE FROM product_extra WHERE productId = ? AND extraId = ?', [
      productId,
      extraId,
    ]);
  }
  ...
  async count(): Promise<number> {
    ...
    const results = await db.select<[{ count: number }]>(
      'SELECT COUNT(*) as count FROM product_extra',
    );
