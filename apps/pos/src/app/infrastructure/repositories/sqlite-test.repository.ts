...
export class SQLiteTestRepository implements BaseRepository<TestEntity> {
  ...
  async findById(id: number): Promise<TestEntity | null> {
    ...
    const result = await db.select<TestEntity[]>(
  ...
  async findAll(): Promise<TestEntity[]> {
    const db = await this.getDb();
    return await db.select<TestEntity[]>(
  ...
  async create(entity: Omit<TestEntity, 'id'>): Promise<TestEntity> {
    const db = await this.getDb();
    const result = await db.execute('INSERT INTO test_entity (name, value) VALUES ($1, $2)', [
      entity.name,
      entity.value,
    ]);
  ...
  async delete(id: number): Promise<void> {
    const db = await this.getDb();
    await db.execute('DELETE FROM test_entity WHERE id = $1', [id]);
  }
