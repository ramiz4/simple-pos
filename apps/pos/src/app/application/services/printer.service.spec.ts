...
import { PrinterConfig, PrinterService, ReceiptData } from './printer.service';
...
interface PrinterServiceInternal {
  config: PrinterConfig;
  prepareReceiptData(orderId: number, language: string): Promise<ReceiptData>;
  generateReceiptESCPOS(data: ReceiptData): string;
}
...
describe('PrinterService', () => {
  ...
  it('should update config', () => {
    ...
    expect(internalService.config.receipt.connection).toBe('tcp:1.1.1.1:9100');
    expect(internalService.config.receipt.width).toBe(42);
  });
  ...
  it('should use web print fallback when not in Tauri', async () => {
    ...
    const windowSpy = vi.spyOn(window, 'open').mockReturnValue(mockWindow as unknown as Window);
  ...
  it('should generate receipt data and format correctly', async () => {
    ...
    const data = await internalService.prepareReceiptData(1, 'en');
  ...
  it('should format ESC/POS correctly for items', () => {
    ...
    const escpos = internalService.generateReceiptESCPOS(mockData);
