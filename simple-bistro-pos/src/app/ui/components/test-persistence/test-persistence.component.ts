      ...
      <div class="max-w-4xl mx-auto">
        ...
        <div class="glass-card p-6 mb-6">
          <h1 class="text-3xl font-bold text-white mb-2">
            Simple Bistro POS - Persistence Test
          </h1>
          <p class="text-white/80">
            Platform: <span class="font-semibold">{{ platformService.isTauri() ? 'Tauri Desktop (SQLite)' : 'Web Browser (IndexedDB)' }}</span>
          </p>
        </div>
        ...
        <div class="glass-card p-6">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-white">Stored Entities ({{ testService.entities().length }})</h2>
            <button
              (click)="refresh()"
                  ...
                  @if (editingId() === entity.id) {
                    ...
                    <div class="flex justify-between items-start">
                      <div class="flex-1">
                        ...
                        <p class="text-white/50 text-sm">ID: {{ entity.id }} | Created: {{ formatDate(entity.createdAt) }}</p>
                      </div>
                      <div class="flex gap-2">
                        <button
...
export class TestPersistenceComponent {
  ...
  async createEntity(): Promise<void> {
    ...
    await this.testService.createTestEntity(
      this.newName.trim(),
      this.newValue.trim() || null
    );
