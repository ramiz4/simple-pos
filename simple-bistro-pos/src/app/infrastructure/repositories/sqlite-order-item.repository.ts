...
export class SQLiteOrderItemRepository implements BaseRepository<OrderItem> {
  ...
  async create(entity: Omit<OrderItem, 'id'>): Promise<OrderItem> {
    ...
    const result = await db.execute(
      ...
      [entity.orderId, entity.productId, entity.variantId, entity.quantity, entity.unitPrice, entity.notes]
    );
  ...
  async update(id: number, entity: Partial<OrderItem>): Promise<OrderItem> {
    ...
    await db.execute(
      ...
      [updated.orderId, updated.productId, updated.variantId, updated.quantity, updated.unitPrice, updated.notes, id]
    );
    return updated;
  }
  ...
  async count(): Promise<number> {
    const db = await this.getDb();
    const results = await db.select<[{ count: number }]>('SELECT COUNT(*) as count FROM order_item');
