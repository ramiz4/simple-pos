...
export class SQLiteUserRepository implements BaseRepository<User> {
  ...
  async findByName(name: string): Promise<User | null> {
    const db = await this.getDb();
    const results = await db.select<User[]>('SELECT * FROM user WHERE name = ? AND active = 1', [name]);
    return results.length > 0 ? results[0] : null;
  }
  ...
  async update(id: number, entity: Partial<User>): Promise<User> {
    ...
    await db.execute(
      'UPDATE user SET name = ?, roleId = ?, pinHash = ?, active = ? WHERE id = ?',
      [updated.name, updated.roleId, updated.pinHash, updated.active ? 1 : 0, id]
    );
    return updated;
  }
