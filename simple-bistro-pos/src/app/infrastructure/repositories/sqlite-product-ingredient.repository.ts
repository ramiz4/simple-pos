...
export class SQLiteProductIngredientRepository implements BaseRepository<ProductIngredient> {
  ...
  async findById(id: number): Promise<ProductIngredient | null> {
    ...
    const results = await db.select<ProductIngredient[]>('SELECT * FROM product_ingredient WHERE id = ?', [id]);
  ...
  async findByProduct(productId: number): Promise<ProductIngredient[]> {
    const db = await this.getDb();
    return await db.select<ProductIngredient[]>('SELECT * FROM product_ingredient WHERE productId = ?', [productId]);
  }
  ...
  async deleteByProductAndIngredient(productId: number, ingredientId: number): Promise<void> {
    const db = await this.getDb();
    await db.execute('DELETE FROM product_ingredient WHERE productId = ? AND ingredientId = ?', [productId, ingredientId]);
  }

  async count(): Promise<number> {
    const db = await this.getDb();
    const results = await db.select<[{ count: number }]>('SELECT COUNT(*) as count FROM product_ingredient');
