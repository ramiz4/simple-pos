...
export class BackupService {
  ...
  async exportBackupToFile(backup: BackupData, filename?: string): Promise<void> {
    ...
    link.href = url;
    link.download = filename || `bistro-pos-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  private async encryptData(data: any, password: string): Promise<string> {
    ...
    try {
      ...
      const key = await crypto.subtle.deriveKey(
        {
          name: 'PBKDF2',
          salt: encoder.encode('simple-bistro-pos-salt'),
          iterations: 100000,
          hash: 'SHA-256'
        },
        keyMaterial,
  ...
  private getCodeTranslationRepo() {
    return this.platformService.isTauri() ? this.sqliteCodeTranslationRepo : this.indexedDBCodeTranslationRepo;
  }
  ...
  private getIngredientRepo() {
    return this.platformService.isTauri() ? this.sqliteIngredientRepo : this.indexedDBIngredientRepo;
  }

  private getProductExtraRepo() {
    return this.platformService.isTauri() ? this.sqliteProductExtraRepo : this.indexedDBProductExtraRepo;
  }

  private getProductIngredientRepo() {
    return this.platformService.isTauri() ? this.sqliteProductIngredientRepo : this.indexedDBProductIngredientRepo;
  }
